...
const page = () => {
  ...
  useEffect(() => {
    const loadBlogs = async () => {
      let req = await fetch("/api/blogs/get-savedArticles?limit=9")
  ...
  const fetchPage = async (pageNumber) => {
    setIsLoading(true)
    let req = await fetch(`/api/blogs/get-savedArticles?page=${pageNumber}&limit=9`)
    let data = await req.json()
    if (data.success) {
      await data.blogs.forEach(blog => {
        blog.date = new Date(blog.createdAt).toDateString();
  ...
  return (
    <div>
      ...
      >
        ...
        <h1 className="text-5xl font-bold text-center">📂 Saved Articles</h1>
        <p className="text-secondaryForeground">{totalSaved} articles</p>
      </motion.div>
            ...
            isLoading
              ...
              : (Articles.length > 0 ?
                ...
                <ArticleCard
                  ...
                  uid={article._id}
                />
              )) : <h2 className="text-2xl text-center text-secondaryForeground min-h-[50vh]">No saved articles found</h2>)
          }
        </div>
